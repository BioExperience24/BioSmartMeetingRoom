...
public abstract class BaseLongUowService<VM, E>(IServiceProvider sp, IMapper mapper)
    : IBaseLongService<VM>
    ...
    where E : BaseLongEntity, new()
{
    ...
    public virtual async Task<IEnumerable<VM>> GetAll()
    {
        ...
        BaseLongUowRepository<E> repo = new(uow);
    ...
    public virtual async Task<VM?> Update(VM viewModel)
    {
        ...
        try
        {
            var entity = await repo.GetById(viewModel.Id.GetValueOrDefault(), true);
            if (entity == null)
            {
                return null;
            }
