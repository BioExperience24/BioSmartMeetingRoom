...
public class UserRepository(MyDbContext dbContext) : BaseLongRepository<User>(dbContext!)
{
    ...
    public async Task<IEnumerable<object>> GetItemsFilteredByEntityAsync(UserFilter? filter = null)
    {
        var query = from user in dbContext.Users
                    from level in dbContext.Levels
                        .Where(l => l.Id == user.LevelId).DefaultIfEmpty()
        ...
        if (filter?.ExceptEmployeeId != null)
        {
            query = query.Where(u => u.user.EmployeeId != filter.ExceptEmployeeId);
        }
